...
spinButton.addEventListener("click", () => {
    ...
    const kqua1 = document.getElementById("kqua1")
    const kqua2 = document.getElementById("kqua2")
    const kqua3 = document.getElementById("kqua3")
    setTimeout(() => {
        if (hits > 0) {
        ...
        li.textContent = `Cược: ${betAmount} VNĐ | Chọn số: ${betNumber} | Kết quả: ${results.join(
            ", "
        )} | Số dư: ${balance}`
        ...
        if (results) {
            kqua1.innerHTML = `Kết quả: ${results[0]}`
            kqua2.innerHTML = `Kết quả: ${results[1]}`
            kqua3.innerHTML = `Kết quả: ${results[2]}`
        }
    }, 4000)
})
